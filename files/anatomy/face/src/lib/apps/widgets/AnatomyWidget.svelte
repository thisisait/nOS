...
<script lang="ts">
	...
	const pos = new Map(
		placed.nodes.map((p) => [p.id, { x: p.rank * RANK_W, y: ((p.y - PAD) / ROW_H) * ROW_PITCH }])
	);
	const VB = {
	...
	<section class="widget glass" aria-label="Anatomy at a glance">
		...
		<footer>
			<p class="rule">
				...
				{spot.edges.length} induced edge(s), {spot.components} component(s). Hover a box for its full
				id, body, state and degree.
			</p>
			<p class="prov">
				graph: build-time artifact ({graph.counts.nodes ?? 0} nodes, regenerated by tools/anatomy-graph-gen.py
				at converge) · pulse state: polled every 60 s
			</p>
			<p class="self">
				{#if selfNode}
