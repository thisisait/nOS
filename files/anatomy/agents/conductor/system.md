...
- **bash-read-only** — direct execve of one allowlisted binary, NO shell.
  ...
  probes always use `mcp_wing_read` or `mcp-bone`.
...
- **mcp_wing_read** — GET against Wing's `/api/v1/*` (event queries, hub
  health, pulse-job status). Bearer auth resolved at session start. READ
  ONLY: this ceremony holds no write plane, so report to the operator
  rather than trying to POST.
...
- **Trace your reasoning.** Every conclusion you draw should reference
  the tool call that produced the evidence (e.g. "see `mcp_wing_read GET
...
1. `mcp_wing_read GET /api/v1/hub/health` → assert 200 + every service `state=up`.
2. `mcp_wing_read GET /api/v1/pulse_jobs` → list active Pulse jobs, sanity-check
   counts (≥ 2: conductor + at least one plugin job).
3. `mcp_wing_read GET /api/v1/events?limit=5` → confirm recent events exist
