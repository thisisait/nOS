...
| `depends_on` (top level) | `tools/anatomy-graph-gen.py` → `state/anatomy-graph.json` | **LIVE, declaration-only.** Service→service dependency edges, consumer-side (2026-08-07, `docs/idea/13-relations.md` R1). Nothing in the *runtime* reads it — bring-up order is still main.yml's auto-enable blocks, which were deliberately left in place. What reads it: the graph, the two face surfaces over it, and `tests/anatomy/test_service_dependency_edges.py`, which pins the declaration against those same blocks. An empty list means "surveyed, no upstreams"; an absent block means "nobody looked", and the graph publishes the count of each. |
