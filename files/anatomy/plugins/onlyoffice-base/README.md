# onlyoffice-base — proxy-auth plugin (Q2 batch)

> **Status:** live, captured 2026-05-04. Wires `pazny.onlyoffice` into the
...
`roles/pazny.onlyoffice/` keeps install-only responsibilities — image pin, port
...
`onlyoffice` is in the second bucket — Authentik enforces login at the
...
Plugin must report `ok` for `onlyoffice-base`.
