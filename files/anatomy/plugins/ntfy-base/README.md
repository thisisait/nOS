# ntfy-base — proxy-auth plugin (Q2 batch)

> **Status:** live, captured 2026-05-04. Wires `pazny.ntfy` into the
...
`roles/pazny.ntfy/` keeps install-only responsibilities — image pin, port
...
`ntfy` is in the second bucket — Authentik enforces login at the
...
Plugin must report `ok` for `ntfy-base`.
