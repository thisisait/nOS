# paperclip-base — proxy-auth plugin (Q2 batch)

> **Status:** live, captured 2026-05-04. Wires `pazny.paperclip` into the
...
`roles/pazny.paperclip/` keeps install-only responsibilities — image pin, port
...
`paperclip` is in the second bucket — Authentik enforces login at the
...
Plugin must report `ok` for `paperclip-base`.
