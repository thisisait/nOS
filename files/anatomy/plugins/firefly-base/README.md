# firefly-base — proxy-auth plugin (Q2 batch)

> **Status:** live, captured 2026-05-04. Wires `pazny.firefly` into the
...
`roles/pazny.firefly/` keeps install-only responsibilities — image pin, port
...
`firefly` is in the second bucket — Authentik enforces login at the
...
Plugin must report `ok` for `firefly-base`.
