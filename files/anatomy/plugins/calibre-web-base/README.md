# calibre-web-base — proxy-auth plugin (Q2 batch)

> **Status:** live, captured 2026-05-04. Wires `pazny.calibre_web` into the
...
`roles/pazny.calibre_web/` keeps install-only responsibilities — image pin, port
...
`calibre-web` is in the second bucket — Authentik enforces login at the
...
Plugin must report `ok` for `calibre-web-base`.
