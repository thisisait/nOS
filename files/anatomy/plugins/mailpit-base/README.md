# mailpit-base — proxy-auth plugin (Q2 batch)

> **Status:** live, captured 2026-05-04. Wires `pazny.mailpit` into the
...
| GDPR Article 30 row | `gdpr:` | `legitimate_interests`, retention 7d |
| Wing /hub deep-link card | `ui-extension.hub_card` | Operator entry point |

## What stays in the role

`roles/pazny.mailpit/` keeps install-only responsibilities — image pin, port
...
`mailpit` is in the second bucket — Authentik enforces login at the
...
Plugin must report `ok` for `mailpit-base`.
