# nodered-base — proxy-auth plugin (Q2 batch)

> **Status:** live, captured 2026-05-04. Wires `pazny.nodered` into the
...
`roles/pazny.nodered/` keeps install-only responsibilities — image pin, port
...
`nodered` is in the second bucket — Authentik enforces login at the
...
Plugin must report `ok` for `nodered-base`.
