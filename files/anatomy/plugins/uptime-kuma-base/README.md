# uptime-kuma-base — proxy-auth plugin (Q2 batch)

> **Status:** live, captured 2026-05-04. Wires `pazny.uptime_kuma` into the
...
`roles/pazny.uptime_kuma/` keeps install-only responsibilities — image pin, port
...
`uptime-kuma` is in the second bucket — Authentik enforces login at the
...
Plugin must report `ok` for `uptime-kuma-base`.
