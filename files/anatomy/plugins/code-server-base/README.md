# code-server-base — proxy-auth plugin (Q2 batch)

> **Status:** live, captured 2026-05-04. Wires `pazny.code_server` into the
> Authentik forward-auth doctrine via Traefik middleware. Tier 2
> (manager) per CLAUDE.md RBAC mapping.
...
| RBAC tier | `authentik.tier: 2` | Maps to `nos-managers` group + above |
| Traefik labels | `compose_extension.template` | `authentik@file` middleware applied |
| GDPR Article 30 row | `gdpr:` | `legitimate_interests`, retention 365d |
| Wing /hub deep-link card | `ui-extension.hub_card` | Operator entry point |

## What stays in the role

`roles/pazny.code_server/` keeps install-only responsibilities — image pin, port
...
`code-server` is in the second bucket — Authentik enforces login at the
...
container port 8443 unless launched with `--cert`. Do NOT add
`code-server` to `traefik_https_upstream_ids` in
`roles/pazny.traefik/vars/main.yml` — Traefik must speak HTTP to the
upstream. The Traefik label set above carries no `*.scheme=https`
entry, so the default HTTP transport applies.
...
Plugin must report `ok` for `code-server-base`.
