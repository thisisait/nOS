# influxdb-base — proxy-auth plugin (Q2 batch)

> **Status:** live, captured 2026-05-04. Wires `pazny.influxdb` into the
...
`roles/pazny.influxdb/` keeps install-only responsibilities — image pin, port
...
`influxdb` is in the second bucket — Authentik enforces login at the
...
Plugin must report `ok` for `influxdb-base`.
