...
- **Security remediation backlog:** authoritative file `docs/llm/security/remediation-queue.json`. As of 2026-07-15 (cycle-16): **13 pending / 104 resolved / 4 vendor-blocked / 1 wontfix** of 122 (REM-118 FreeScout CVSS-9.4 unauth takeover → nfrastack 2.1.3, REM-110 Bone unauth recon scope-gate, REM-107 Alloy OTLP loopback — all resolved + live-verified 2026-07-08). **Cycle-17 batch-38 (2026-07-30) reopened the CRITICAL column — the "0 CRITICAL pending" claim carried here for two weeks was re-derived by nobody and is now false.** Live queue: **2 pending CRITICAL** — REM-137 (Gitea 1.27.0, 36-CVE cluster) and REM-144 (Traefik dashboard/API anonymously reachable at the edge; `/api/http/middlewares` served both SEC-6 edge tokens verbatim, one of which was `{prefix}_pw_face_edge` and so disclosed the global password prefix). REM-144 fixed 2026-07-30 (`67792f0c`: `traefik_skip_ids` + `face_edge_token` off the prefix + a justification-field gate); REM-145 (GHSA-3ccp-42pg-hgv6) rode along as v3.6.24. The rest of the burn-down is a **version-pin drift wave** — authoritative forward plan is `docs/roadmap.md`; the structural follow-up (one SoT for exposure/RBAC/compliance) is `docs/plans/nos-genome-and-organelles.md`. Phase A is mechanical CVE pins; Phase B is `mem_limit`/`cpus` sweep (done); Phase C is hardening; Phase D is architectural. Vendor-blocked: **FreePBX REM-014/046/113** (tiredofit Docker image abandoned upstream 2022-04-30 — CRITICAL CVEs UNFIXABLE in this image, operators accept the risk) **+ Ollama REM-126** (unauthenticated GGUF-quantization memory leak, no upstream fix); REM-064 (Open WebUI, authenticated-admin-only RCE) reclassified **wontfix**, REM-059 (RustFS) pending.
