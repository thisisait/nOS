...
- **CI Integration uses an isolated ansible venv — the "filter-less" CI red was a runner split-brain (2026-06-08):** the whole 2026-06-08 CI saga (cascade of *"No filter named 'bool'/'to_nice_json'/…"* + `| default()` not trapping undefined, Integration jobs only) had a **single root cause: the GitHub runner ships a system ansible-core baked into a *different* Python than the one CI pins.** Bare `ansible-playbook`/`ansible-galaxy` resolved to that baked (newer) ansible while our pinned deps landed in the toolcache Python — a split-brain whose symptom is a mismatched ansible tree (a newer `core.py` importing `VaultDecryptionContext` over an older `_dumper.py` that lacks it → `ansible.builtin.core` filter plugins **skipped** → EVERY filter, incl. `default`, throws "No filter named"). It was **not** ansible-core 2.20.6, **not** Python 3.14, **not** the deprecated `{{ vars }}` path, **not** ansible.cfg — all earlier misdiagnoses (a dev-box venv is single-version so it can't reproduce; pytest jobs stayed green because they don't run the playbook). **Fix:** the two Integration jobs install ansible into a fresh `python -m venv "$RUNNER_TEMP/nosvenv"` and prepend its `bin` to `$GITHUB_PATH`, so `ansible-playbook`/`ansible-galaxy`/`ANSIBLE_PYTHON_INTERPRETER` all point at one clean, single-version install (`.github/workflows/ci.yml`). A post-install probe (`ansible -m debug -a "msg={{ … | to_nice_json }}"`) fails fast if core filters don't load. The `<2.20.6` ceiling + `python-version: '3.13'` pins are kept as baseline-matching but are NOT load-bearing for this fix. The 5 offline gates in `tests/anatomy/test_config_stock_jinja_only.py` + the stock-Jinja edits made while chasing this are kept as defensive portability hardening (behaviour-preserving on 2.20.5/Py3.13).
...
- **Security remediation backlog:** authoritative file `docs/llm/security/remediation-queue.json`. As of 2026-06-06: **16 pending / 69 resolved / 2 vendor-blocked** out of 87 total (Epic C1 image-pin sweep burned down the version-bump backlog; REM-043 n8n SSRF closed in the v0.5 security cluster). Phase A is mechanical CVE pins; Phase B is `mem_limit`/`cpus` sweep; Phase C is hardening; Phase D is architectural. Vendor-blocked: Open WebUI ZDI CVEs (REM-064), RustFS gRPC sigverify (REM-059), **FreePBX REM-014/046** (tiredofit Docker image abandoned upstream 2022-04-30 — CRITICAL CVEs UNFIXABLE in this image, operators accept the risk).
