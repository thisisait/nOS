...
- **Track Q autowiring debt** (4-6 weeks, post-PoC) — **UNBLOCKED + Q1 PARTIAL 2026-05-04**. A6.5 (Grafana thin-role pilot) landed + Q1 batch shipped in the same wave: `prometheus-base`, `loki-base`, `tempo-base` plugins now own their master configs + recording rules. core-up.yml dropped 4 deploy tasks (Prom + rules + Loki + Tempo). Plugin loader gained `render_dir` action for whole-directory renders. Remaining Q1: alloy host-side (different pattern, Homebrew brew service) + 8-12 composition plugins (grafana-prometheus / grafana-loki / grafana-tempo cross-wiring) → Q1b. 30 → 26 of 71 `pazny.*` roles still carry `tasks/post.yml` cross-service wiring (Grafana + Prom + Loki + Tempo cleared).
