...
- **Security remediation backlog:** authoritative file `docs/llm/security/remediation-queue.json`. As of 2026-06-10: **14 pending / 71 resolved / 2 vendor-blocked** out of 87 total (Epic C1 image-pin sweep burned down the version-bump backlog; REM-043 n8n SSRF closed in the v0.5 security cluster; REM-077 traefik encodedCharacters + REM-006 mem/cpu sweep verified-implemented and closed by the 2026-06-10 serial review). Phase A is mechanical CVE pins; Phase B is `mem_limit`/`cpus` sweep (done); Phase C is hardening; Phase D is architectural. Vendor-blocked: Open WebUI ZDI CVEs (REM-064), RustFS gRPC sigverify (REM-059), **FreePBX REM-014/046** (tiredofit Docker image abandoned upstream 2022-04-30 — CRITICAL CVEs UNFIXABLE in this image, operators accept the risk).
- **OpenTofu post-cutover:** punch list #1–#3 shipped 2026-06-12 (backup custody via `run_tofu_state()`, enabled-filtering in registry→tfvars — NOTE: a service flipping `enabled→false` on a live tenant plans as DESTROY and the guard refuses until a supervised apply or the next blank — and the daily plan-only drift Pulse job `authentik-tofu-drift-base`). Still open: adopt-path attachment import id (existing-tenant migrations only). Authoritative: `docs/opentofu-authentik-cutover.md` § Open items.
