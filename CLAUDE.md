...
- **Track Q autowiring debt** — **Q1+Q2 + D1 (central retirement) DONE 2026-05-05; Q3+Q4+Q5+Q7 DONE 2026-05-07**. Phase 1 multi-agent batch shipped 33 plugins (Q1b composition + Q2 native_oidc/forward_auth). D1 series (2026-05-05) closed the central `authentik_oidc_apps` refactor: D1.0 fixed 4 mode mis-classifications (erpnext/HA/jellyfin/superset → native_oidc), β1.A added the `header_oidc` doctrine bucket + reclassified firefly, β1.B implemented true native OIDC for Node-RED via passport-openidconnect (first non-trivial native SSO upgrade — formerly forward_auth-only), β1.C/D documented metabase OSS limitation + roadmap for upstream PRs (`docs/upstream-pr-opportunities.md`), D1.1 added spacetimedb-base stub closing the last C1 blocker, D1.2 extended `run_aggregators` with template_vars (Jinja pre-render + feature_flag gating) and switched the 10-oidc-apps + 20-rbac-policies blueprints to iterate `inputs.clients`, D1.3 retired the 35-row central list (only an empty stub remains as Tier-2 apps_runner channel). Aggregator dry-run (`tools/aggregator-dry-run.py`) gates the cutover with 0 field-diffs. Tier-1 SSO source-of-truth = per-plugin authentik: blocks. **Q3/Q4/Q5/Q7 (2026-05-07):** 4-worker parallel batch (≤5 doctrine throttle, no 3-worker trial needed under cap) shipped 12 headless-infra base manifests for the last role-only services — Q3 storage+DB (postgresql/mariadb/redis/rustfs/bluesky-pds), Q4 comms (freepbx/smtp-stalwart), Q5 content (mcp-gateway/offline-maps/qgis-server), Q7 edge+ops (traefik/watchtower). All mirror the prometheus-base shape (no `authentik:`, no `compose_extension:` — substrates / forward-auth / no-SSO). Loader now discovers 62 plugins (was 50). Follow-up commit `f2ba31b` dropped `noop:` action keys from postgresql/mariadb/redis (loader returns `unknown:noop` for unrecognized actions — alloy-base/watchtower-base canonical shape is to omit `post_compose:` entirely when no HTTP probe). Multi-agent doctrine held: zero cross-leak, all 4 workers wrote only to their own plugin dirs, main worktree clean throughout. **D2 next:** delete duplicate role-side OIDC env blocks (compose templates) — plugin compose-extensions are the live render path now. Long-term: extend aggregator with `from: app_manifest` source so Tier-2 apps land in inputs.clients alongside Tier-1.
...
- **SSO + identity attribution doctrine — locked 2026-05-17:** mode trichotomy (`native_oidc` / `header_oidc` / `forward_auth`) pinned by anatomy gates in `tests/anatomy/test_sso_doctrine.py`. Non-canonical labels rejected (was: gitea=oauth2, qdrant=proxy_auth, grafana=no-mode — all 3 normalized). `requireSuperAdmin()` now accepts BOTH `nos-providers` AND `nos-admins` (was nos-providers only, 403'd akadmin operator). **Body-attribution privilege-escalation fix:** GitleaksPresenter::actionResolve + RemediationPresenter::actionBulkStatus pulled `$body['resolved_by']` from client; now `getActorId()` from validated bearer token + explicit 400 reject if body carries the field. Sweep anatomy gate forbids the anti-pattern on every Api/* presenter. **Direct attribution on pentest_findings + pentest_targets:** new `discovered_by` / `resolved_by` / `created_by` columns + ALTER sweep for legacy DBs. **Agent runners:** Scout (full — drift/visibility), Remediator (full — A9.3), Conductor (full — A8). Inspektor + Librarian ship as AgentKit-contract-only with explicit `metadata.runner_status: deferred` (waiting on trivy/grype/nuclei substrate + Qdrant corpus pipeline respectively). Authoritative guide: `docs/sso-and-attribution.md`. 19+ anatomy gates across `test_sso_doctrine.py` + `test_scout_inspektor_librarian.py`.
- **Notification fanout — A9 (2026-05-16, extended through 2026-05-17):** operator-attention queue + per-plugin severity routing. `wing.db.notifications` table; `App\Model\NotificationRepository` + DI; Bone `POST/GET /api/v1/notifications` (HMAC) + `clients/wing.py::insert_notification` with routing fallback; Wing /inbox promoted to operator-attention surface (mark-read POST-only). PHP CLI dispatch worker `bin/dispatch-notifications.php` flushes ntfy + SMTP (mailpit dev OR Stalwart STARTTLS/AUTH LOGIN production), registered as a per-minute Pulse subprocess job in `wing-base/plugin.yml`. `wing-base` aggregator harvests every peer's `notification:` block + each agent profile's block; routing JSON sidecar `~/wing/app/data/notification-routing.json` is the runtime source. **A9.2 daily-digest** ships severity-floor bucketing + daily aggregated email flush (separate Pulse cron). **A9.3** genericized pulse-run-agent.sh to NOS_AGENT_*; added Remediator agent (read-only security triage). **A9.4** added Scout (drift/visibility agent, full) + Inspektor + Librarian (contract-only — runners pending tooling). **A9.5** per-plugin notification templates: plugin manifest `notification.templates.<name>: {title, body}` with $var placeholders; Bone renders via Python string.Template.safe_substitute. **Stalwart TLS SMTP path** (was deferred): wing-base plugin manifest now flips MAIL_TLS_MODE + AUTH credentials when install_smtp_stalwart=true; `_smtp_open_session()` helper handles raw/STARTTLS/implicit modes. **Live consumers:** gitleaks skill (new findings); conductor (non-zero exit); remediator (triage report). 24+ anatomy gates in `tests/anatomy/test_notification_fanout.py` + 7 in `test_notification_templates.py`. Authoritative guide: `files/anatomy/docs/notification-fanout.md`.
