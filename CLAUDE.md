...
### Role-based service delivery (72 roles under `pazny.*`)
...
- **Security remediation backlog:** authoritative file `docs/llm/security/remediation-queue.json`. As of 2026-07-08 (cycle-16): **36 pending / 79 resolved / 3 vendor-blocked** of 118 (REM-118 FreeScout CVSS-9.4 unauth takeover → nfrastack 2.1.3, REM-110 Bone unauth recon scope-gate, REM-107 Alloy OTLP loopback — all resolved + live-verified 2026-07-08). The dominant remaining burn-down is a **29-item version-pin drift wave** (2 CRITICAL: Gitea 1.25 EOL REM-099, Woodpecker misconfig REM-002) — authoritative forward plan is `docs/roadmap.md`. Phase A is mechanical CVE pins; Phase B is `mem_limit`/`cpus` sweep (done); Phase C is hardening; Phase D is architectural. Vendor-blocked: **FreePBX REM-014/046/113** only (tiredofit Docker image abandoned upstream 2022-04-30 — CRITICAL CVEs UNFIXABLE in this image, operators accept the risk); REM-064 (Open WebUI) + REM-059 (RustFS) reclassified pending.
