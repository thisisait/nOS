...
- **ansible-core 2.20.6 ceiling pin + `{{ vars }}` refactor (2026-06-08):** CI pins `ansible-core>=2.20,<2.20.6` (4 sites in `.github/workflows/ci.yml`) — **NOT** `<2.21`. **2.20.6 regressed the deprecated `{{ vars }}` path:** after the first `template_vars: "{{ vars }}"` finalization (core-up plugin loader, `tasks/stacks/core-up.yml:38`), the Jinja env goes **filter-less for the rest of the run** — every non-stock filter (`bool`, `to_nice_json`, `regex_replace`, `b64encode`, …) throws *"No filter named"*, and `| default()` stops trapping undefined vars. It is a **global** degradation and is **NOT reproducible locally** (full vars, exact constructs, real module, both ansible.cfgs, deprecation on/off all clean on a dev box — only a full CI-runner run triggers it); ruled out: `deprecation_warnings` (fires locally without degrading), `hostvars[inventory_hostname]` (lossy — drops templated config vars like `stacks_dir`, so NOT a `{{ vars }}` drop-in), ansible.cfg. 2.20.5 is clean everywhere. The **real fix is the strategic `{{ vars }}` refactor** (stop passing the whole namespace at the `template_vars`/`role_vars` sites — `{{ vars }}` is itself removed in 2.24) — a proper follow-up that needs a complete-var-dict mechanism, tied to the 2.24 jump above. Until then: stay on 2.20.5. The 5 offline gates in `tests/anatomy/test_config_stock_jinja_only.py` (filters + ref-resolution + loop-item + meta-when) are kept — they harden the code for both 2.20.5 and a future 2.20.6+/2.24.
