...
**Cloud e2e lane (Claude Code on the web, 2026-09-25):** a cloud session is a disposable Ubuntu 24.04 container — root, **no systemd**, no `$USER`, shared egress IP, Galaxy/ghcr-blobs/quay/lscr refused. `.claude/hooks/session-start.sh` → `tools/cloud/bootstrap.sh` (frozen venv, Galaxy→git fallback, dockerd + `mirror.gcr.io`); then `tools/cloud/e2e.sh static|preflight|converge|smoke|idempotence|all` with `profiles/cloud-e2e.yml` — the DEFAULT config minus what the sandbox's network/disk refuse (each line says which). Platform differences are facts, not toggles ([docs/cross-platform.md](docs/cross-platform.md): `nos_init`, `nos_is_container`, `nos_container_user`, `nos_kernel_ipv6`). Host daemons run under `nos-proc` when `nos_init == 'none'`. Runbook + measured sandbox facts: [docs/cloud-e2e.md](docs/cloud-e2e.md).
...
### Role-based service delivery (roles under `pazny.*` — count moves, ask ls)
...
- **`native_oidc`** — service consumes OIDC at app level. Operator clicks "Sign in with Authentik" inside the service. Per-user identity flows into the service.
  ...
  - **FreeScout — forward_auth since REM-192 (2026-08-11); the native-OIDC path was removed 2026-09-03 (fee 49).** Both module sources were HTTP 404 and the env block rendered a secret nothing consumed.
...
- **Agent roster — do not carry it in prose.** The sentence that stood here ("Scout, Remediator, Conductor are live") was wrong in both directions by 2026-08: scout and remediator had zero events in the wing.db epoch while surveyor and librarian ran hundreds. Ask `tools/agent-status.py` (what the agents did, how the runs ended) and `tools/identity-status.py` (declared clients vs realms) — the same rule as the security queue and the plugin-wiring tally. What is knowledge rather than state: the 2026-08-26 roster close RETIRED scout / remediator / upgrade-advisor (history keeps them; upgrade-ARCHITECT is a different, live agent) and PARKED inspektor / curator / migration-author as AgentKit contract-only (`metadata.runner_status`, Authentik client reserved, no launcher/pulse apparatus — a parked agent says so in its own `agent.yml`).
...
- **Tables** (in `files/anatomy/wing/db/schema-extensions.sql`): `agent_sessions`, `agent_threads`, `agent_iterations`, `agent_vaults`, `agent_credentials`, `agent_subscriptions` (verified by `tests/anatomy/test_agentkit_naming.py::test_all_agentkit_tables_declared`). There is **no agent-memory table**: Q8=c (2026-08-28) deleted the Dreams subsystem entirely — KEAP is the estate's memory, and a second memory beside the cortex is a second truth. Pinned by `tests/anatomy/test_agent_memory_does_not_return.py`.
...
- **Outcome iteration**: satisfaction is written by a GATE RUN (`outcomes.gateset`), never a model (392fd6ee); `rubric_path`/grader are feedback-only steering revision. Max 3 iterations default, 10 max.
...
- Multi-agent process pool + Dreams memory — **both DELETED 2026-08-28**. Every manifest is `multiagent.type: solo`, so `Coordinator.php`/`ProcessPool.php` were ~800 unreachable lines; agent memory went by operator answer Q8=c. Gate against the return: `tests/anatomy/test_agent_memory_does_not_return.py`. The `multiagent:` manifest block and `Agent::isCoordinator()` survive as manifest surface with no runtime behind them.
