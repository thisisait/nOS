...
- **Security remediation backlog:** authoritative file `docs/llm/security/remediation-queue.json`. **Do not quote these numbers — re-derive them.** This line has now been wrong three times by inheritance, and the third time it was a machine that noticed: `discovery:contradiction-scan` (nightly, first run 2026-08-06) compares the queue against `docker ps` AND against this paragraph, and reported *"CLAUDE.md's backlog tally disagrees with the queue (2 field(s))"*. Read its findings before editing this line by hand. As of 2026-08-07 (cycle-24, re-derived from the file): **29 pending / 135 resolved / 5 vendor-blocked / 4 wontfix / 1 obsolete** of 174; pending by severity **0 CRITICAL / 5 HIGH / 13 MEDIUM / 11 LOW**. The HIGHs: REM-150 (mcp_gateway), REM-159 (gitlab 18→19, major), REM-165 (erpnext), REM-168 (homeassistant, a config change not a bump), REM-176 (openwebui 0.10.2 → 0.11.0). **0 CRITICAL is not the same as none found** — twelve items so far were already LIVE at their fix version and had simply never been reconciled (REM-136/137/139/140/141/142, then REM-138 and REM-151 on 08-06, then REM-129/148/152/160 on 08-07). The queue does not learn from a converge, so a "pending" row may only be stale. **`discovery:contradiction-scan` is a READER, not a writer** — it compares the queue against `docker ps` and files a roadmap row; it does not close anything, and a claim that it does (made here on 2026-08-06) is wrong. Closing stays a deliberate act with the evidence written into `resolved_by`. **A queue row can also be wrong in the other direction:** REM-178 found REM-137 recording fix `1.27.0` while the pin and the running binary were `1.27.1` — a recorded fix BELOW what the estate runs re-opens the gap if anyone acts on it. Both former CRITICALs are now live-verified against `docker ps`: REM-137 (Gitea `1.27.0`, 36-CVE cluster) and REM-153 (Metabase `v0.61.9`, `GHSA-cwxq-fmxq-jv8h`). Cycle-21's own two HIGHs to weigh: **REM-151** — `global_password_prefix` still defaults to `changeme` and the weak-prefix assert does not cover local tenants, i.e. the other half of the REM-144 surface — and **REM-152** (n8n 17-GHSA wave, `2.28.1` → `2.32.7`). **The methodological lesson of cycle-19/20, and it recurred three times in one batch: a GHSA with NO CVE id.** A scan phrased as "no new *CVE* past the pin" was literally true and wrong by six days (metabase), and the same blind spot produced the n8n 17-GHSA and authentik waves — scan the vendor's advisory endpoint, not the CVE feed. The rest of the burn-down is a **version-pin drift wave** — forward plan `docs/roadmap.md`; the structural follow-up (one SoT for exposure/RBAC/compliance) is `docs/archive/nos-genome-and-organelles.md`. Vendor-blocked: **FreePBX REM-014/046/113** (tiredofit image abandoned upstream 2022-04-30 — CRITICAL CVEs UNFIXABLE, operators accept the risk) **+ Ollama REM-126** (unauthenticated GGUF-quantization memory leak, no upstream fix); REM-064 (Open WebUI, authenticated-admin-only RCE) reclassified **wontfix**.
