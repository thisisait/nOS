...
- **CI Python pinned to 3.13 — the "filter-less" CI red was Python 3.14 (2026-06-08):** the whole 2026-06-08 CI saga (cascade of *"No filter named 'bool'/'to_nice_json'/…"* + `| default()` not trapping undefined, only in the Integration jobs, only on a CI runner) had a **single root cause: Python 3.14**. `setup-python`'s `'3.x'` floated to 3.14.5, where **ansible-core 2.20.x's filter plugins fail to import** (`cannot import name 'VaultDecryptionContext' from 'ansible._internal._yaml._dumper'`) → `ansible.builtin.core` filters are **skipped**, so EVERY Jinja filter (incl. `default`, which is why undefined vars stopped being trapped) throws "No filter named" at template-resolution time. It was **not** ansible-core 2.20.6, **not** the deprecated `{{ vars }}` path, **not** ansible.cfg — those were misdiagnoses (a dev box on Python 3.13 can't reproduce it; pytest jobs stayed green precisely because they pin 3.13). **Fix:** `python-version: '3.13'` at all 7 `setup-python` sites in `.github/workflows/ci.yml`. The `ansible-core>=2.20,<2.20.6` ceiling is kept only to match the operator baseline (2.20.5) — it is NOT load-bearing for this fix; un-restrict to `<2.21` whenever convenient. **Don't bump CI Python past 3.13 until ansible-core ships a release that imports cleanly on 3.14+** (tie to the 2.24 jump above). The 5 offline gates in `tests/anatomy/test_config_stock_jinja_only.py` (filters + ref-resolution + loop-item + meta-when) and the stock-Jinja edits made while chasing this are kept as defensive portability hardening — behaviour-preserving on 2.20.5/Py3.13, and they reduce blast-radius if filter plugins ever fail to load again.
