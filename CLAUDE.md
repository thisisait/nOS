...
- **Track Q autowiring debt** (4-6 weeks, post-PoC) — **UNBLOCKED 2026-05-04** by A6.5 (Grafana thin-role pilot landed). 30 of 71 `pazny.*` roles still carry `tasks/post.yml` cross-service wiring totalling ~3000 LOC. Plan in `docs/bones-and-wings-refactor.md` §13.1 + `files/anatomy/docs/role-thinning-recipe.md`. Q1 (observability batch — prometheus/loki/tempo/alloy after grafana) is the first batch; Grafana itself is the in-repo proof example.
