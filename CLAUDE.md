...
- **OpenTofu post-cutover punch list:** tfstate/tfvars secrets custody (Infisical + 3-2-1 + restore-verify), disabled-service filtering in the registry→tfvars path (tofu currently creates SSO objects for `install_*: false` services — live: erpnext/spacetimedb/mailpit), drift Pulse job (plan-only → notification), adopt-path attachment import id (existing-tenant only). Authoritative list: `docs/opentofu-authentik-cutover.md` § Open items.
