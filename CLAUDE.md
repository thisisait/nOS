...
- **Notification fanout — A9 (2026-05-16):** operator-attention queue + per-plugin severity routing. `wing.db.notifications` table; `App\Model\NotificationRepository` + DI; Bone `POST/GET /api/v1/notifications` (HMAC) + `clients/wing.py::insert_notification` with routing fallback; Wing /inbox promoted to operator-attention surface (mark-read POST-only). PHP CLI dispatch worker `bin/dispatch-notifications.php` flushes ntfy + raw SMTP (mailpit), registered as a per-minute Pulse subprocess job in `wing-base/plugin.yml`. `wing-base` aggregator harvests every peer's `notification:` block + each agent profile's block via the loader's new agent_profile slug stamping (load_plugins.py:333-340); routing JSON sidecar `~/wing/app/data/notification-routing.json` is the runtime source. **First two live consumers:** gitleaks skill emits ONE summary notification per new-finding batch (HMAC via WING_EVENTS_HMAC_SECRET); conductor `pulse-run-agent.sh` emits a critical/high notification on non-zero exit (env error → critical, actionable findings → high). 17 anatomy gates in `tests/anatomy/test_notification_fanout.py`. Authoritative guide: `files/anatomy/docs/notification-fanout.md`. **Deferred:** Stalwart TLS SMTP path (Track G follow-on), daily-digest mail, per-notification idempotency key for webhook dup-suppression. Currently each notification is its own row + dispatch; cheap when queue is empty (dispatch worker no-ops with zero rows).
