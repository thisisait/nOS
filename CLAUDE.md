...
- **Phase 1 multi-agent batch retro (2026-05-05)** — **WORKTREE CROSS-LEAK BUG**. 10 worker agents spawned in parallel git worktrees with `isolation: "worktree"`. Workers occasionally wrote files to the PARENT worktree (not their isolated path), or saw uncommitted edits from sibling workers on first checkout. Concrete failures observed: U6 wrote 7 plugin dirs directly to the main worktree (visible as untracked there post-quota); U7 committed onto `feat/u9-wing-docblocks-a` branch; U9 reported pre-existing dirty state from U6+U10 on its first checkout (reverted + re-applied own scope). Workaround applied: operator manually consolidated all branches/worktrees + uncommitted files into linear master commits. **Investigate the worktree isolation mechanism BEFORE the next multi-agent batch (Phase 3 alloy consumers U11-U13 + A8 conductor sub-batches).** Until fixed, run agents serially or constrain parallelism to non-overlapping file scopes only.
