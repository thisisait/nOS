...
row("plat-defaults-derive","Defaults derive from the graph; config only adds","2026-09-02","next","platform",parent="plat-organ-identity",
    ...
    body="MEASURED 2026-09-01. default.config.yml ships 50 true / 41 false — not a minimal estate. config.yml flips 24 ON, 2 OFF and restates 30 as no-ops, so it is already 12:1 additive. A profile that disables via extra-vars therefore collides with the prune guard: `nos -e @profiles/dev-minimal.yml` REFUSES at core-up because 25 services resolve false for the run while enabled on disk, and `false` means both `never wanted` and `not this run`. prune_disabled_overrides exists only to arbitrate that ambiguity. "
         "THE FIX IS POLARITY, NOT ANOTHER FLAG. install_<svc> defaults true iff layer in {L0,L1} or stack == host — about 13 services, DERIVED from the dependency graph so defaults cannot drift from it. config.yml becomes additive and never carries false. The enabled set is then always a declaration, prune_disabled_overrides is deleted, profiles become additive, and dev-minimal stops existing because it IS the default. "
         "THE PREREQUISITE IS A SURVEY, and it is the bulk of the work: layer covers 26 of 65 services (L0 3, L1 4, L2 19) and 39 carry layer_withheld. A derived default set over a 40%%-populated axis would enable by accident. "
         "ORGAN IS THE WRONG AXIS FOR THIS and the distinction is the point: apex `publish:` assigns 63/65 services to 13 organs, but archive holds cortex and keap beside kiwix and calibre-web, and spine holds bone and wing beside portainer. Organ answers what a service is FOR; only layer answers what the estate cannot start without.")
