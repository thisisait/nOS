...
row("notify-supersede","A notification that stopped being true has no way to stop being unread","2026-08-23","next","platform",
    ...
    body="76 unread, oldest 29 days, and 60 of them repeating classes (os-resume 31, backup 25, prometheus-alert-relay 10, security-drift 4) where each new send makes the previous one false. WRITTEN 2026-08-23: a third state `superseded_at`, never `read` — nobody read them, and stamping read would be the estate lying to itself about a human. Opt-in and emitter-declared (`supersede_key`): inferring the class from origin_plugin would be wrong for exactly the emitters that most need the inbox, since two gitleaks findings are two secrets. Four emitters opted in; prometheus-alert-relay did NOT — it has its own RESOLVED pairing and bin/reconcile-inbox.php handles it. Distinct from that reconciler: it answers *did the condition clear*, this answers *has this news been re-said*. UNVERIFIED until a converge runs the ALTER sweep on the 979 MB live DB.")
