...
row("loop-checkpoint-recover", "Long-horizon: steal the checkpoint+planner, keep the gate",
    "2026-09-04", "queued", "agents", parent="sere",
    refs="github.com/AMAP-ML/LongHorizon-Harness (MIT) · docs/plans/agentic-upgrade-migration-coexistence.md · docs/idea/11-agentic-loop-contract.md §5 · docs/hidden_fees/50-a-status-that-froze-as-unbuilt.md · files/anatomy/wing/app/AgentKit/Runner.php",
    body="DIG-DEEPER, not a port. Read 2026-09-04 against what the loop already is. LongHorizon's own thesis — 'the model determines what an agent can do in one round; the harness engineers the loop around it' — is idea/11 §5 verbatim (bounded, because unbounded is the failure mode), reached independently, which is the signal it is worth reading. Its three roles (Manager plans a bounded step / Executor acts in a fresh context / Auditor independently verifies and refuses the executor's claims) map onto proposer→judge→review, and its 'only audited results become trusted state' IS 'satisfaction written by a gate, never a model'. "
         "WHERE WE ARE ALREADY STRONGER, so nobody re-buys it: their Auditor is an LLM reading files/logs/tests; ours is a deterministic gate run (pytest, ansible-lint, nos-smoke, corpus-diff) with ratchets, oracle-path budgets and sandboxed worktrees. For CODE that is a harder guarantee than an LLM verifier — do NOT replace the gate with an LLM auditor. Note their headline benchmarks (WeaveBench 52->81, OSWorld 2.8->8.3, Terminal-Bench 70->77) are GUI/computer-use tasks, a domain where no deterministic gate exists and an LLM auditor is the only option; ours is estate self-improvement, where it is not. Also already ours: the ledger, fingerprint dedup, path budgets, apex, per-agent model binding (their per-role selection), the LLMClient adapter layer (their agent-agnostic backends), baseline-gate-attribution. "
         "THE ONE GENUINELY ABSENT THING is a DURABLE VERIFIED CHECKPOINT that a FRESH CONTEXT resumes from, and a MANAGER that rebuilds task state each round from (original goal + verified progress + failure evidence) to plan the next bounded step. We named this gap ourselves and have not built it: idea/11 admits 'engine shipped / between-session carry absent' (fee 50), the Dreams memory subsystem was deleted (Q8=c), and the Runner iterates WITHIN a session by replaying the conversation under a ceiling — there is no checkpoint a new context restarts from. Their model is exactly that missing layer: a step fails, the failure becomes durable evidence, a refreshed Manager re-plans from the last VERIFIED checkpoint rather than from a rotting context. "
         "WHERE IT LANDS: this is the orchestration layer for the front-queued agentic-upgrade->migration->coexistence epic (PG 16->17), which is inherently multi-step (pre/apply/post/rollback across services) and which our one-weakness->one-proposal loop has no planner or checkpoint for — the conductor is a reporter, not a planner. DECIDE FIRST, before any code: where a verified checkpoint lives (a new wing.db table? events keyed to a task uuid? — NOT a second memory beside KEAP, Q8's rule), whether the Manager is a new agent or a mode of the conductor, and how failure-evidence is carried without letting a model author its own 'verified progress' (the gate stays the writer of what is verified). "
         "TWO SMALLER STEALS. Mid-run instruction injection: a way for the operator to hand a running session a correction without killing it — we have supervision but not in-flight steering. And fresh-context-per-step as a deliberate context-hygiene strategy against the rot our replay-under-ceiling accumulates. Both are cheap next to the checkpoint layer and independent of it.")
...
row("plat-gate-shape","91 gates assert on source text; one is proven unable to fail","2026-09-02","next","platform",
    ...
    body="PROVEN 2026-09-01, not argued: deleted the whole `selectattr('container_name','defined')` harvest from prune-disabled.yml — the logic the gate exists to protect — and test_prune_disabled_guard stayed GREEN, 15 passed. The assert is `\"container_name\" in TASK.read_text()` and the surviving explanatory COMMENT at line 174 satisfies it. Pinned-name containers would survive their prune, which is the exact failure the gate's own message describes. "
         ...
         "PROGRESS 2026-09-04 (39 logic-file gates swept): of the class, THREE MORE were PROVEN — a comment or unrelated line satisfies the assert today even with the guarded logic deleted — and are fixed + retro-verified: test_agent_clients_blueprint_is_force_applied (30-agent-clients in a comment AND the for-bp loop; now reads the loop, comment-stripped), test_reapply_handler_includes_mfa_blueprint (enforce_mfa in a comment + an unrelated lockout guard, so it could not tell conditional from unconditional apply; now RENDERS the loop with the flag on AND off), test_agent_exit_verdict_sentinel_propagated (all three substrings in comments + an unrelated -gt 0; now matches the real grep + gate). "
         "~55 substring checks were ruled LEGITIMATE (declaration files where the string IS the fact, yaml/json-parsed structural checks, regex-scoped extractions, comment-stripped checks). FIVE more are plausible-but-unproven (no decoy exists TODAY): test_wing_tracy_secret_redaction, test_cortex_organ_contract, test_host_daemon_source_restarts (pulse probe wiring), test_app_wiring (onlyoffice per-task gate), test_mariadb_client_tls (src.count over comments). Left deliberately — fixing an unproven gate is a speculative rewrite; close them when a decoy actually lands or when someone touches the file.")
...
row("authentik-blueprint-readers","Four blueprints still apply under || true with no read-back","2026-09-03","next","platform",
    refs="docs/hidden_fees/46-an-apply-that-always-succeeds.md · tasks/verify-authentik-apps.yml · main.yml (reapply handler)",
    body="Fee 46's open half: 20-rbac-policies, 30-agent-clients, 40-enrollment-flow and 46-brand-auth-flow apply best-effort under BOTH engines (tofu never owns them) and every existing test checks the TEMPLATE renders, not that the apply landed — the same silent class that hid the 3-month Linux EACCES. FIX: extend the existing reader — one GET per authoritative endpoint (policy bindings count, oauth2 providers vs the declared agent roster, the enrollment flow instance, the brand row), one assert each, engine guard relaxed for the six tofu never owns.")
row("organ-health-carries-a-version","Bone and Wing answer health without naming their tree","2026-09-03","next","platform",
    refs="tools/estate-status.py (UNREADABLE row + exit 2) · files/anatomy/bone/ · files/anatomy/wing/",
    body="Verified 2026-09-03: estate-status already fails closed (reachable-no-version = UNREADABLE, exit 2 — the audit's fee-candidate 53 was FALSE about the reader). What remains is the feature: the two primary organs cannot answer `repo != running system` because their health payloads carry no version. FIX: the role stamps a git ref/semver at deploy (a VERSION file or plist env), /health returns it, estate-status's existing version_keys pick it up and the permanent ? becomes a real comparison.")
row("docs-czech-and-fork-residue","Translate the live Czech plan; trim the fork relic","2026-09-03","queued","platform",
    refs="docs/sso-autologin-plan.md · full-mac-setup.md · tasks/macos-defaults.yml:4",
    body="Verified 2026-09-03: sso-autologin-plan.md is 320 lines of Czech (doctrine retired Czech) and claims the sso_autologin* vars do not exist while they shipped the same day (6664895c, dormant behind false) — translate, fix the stale claim, archive if fully implemented. full-mac-setup.md is geerlingguy-fork residue (Dropbox, Photoshop, /Users/jgeerling paths) but tasks/macos-defaults.yml:4 cites its manual-settings list — trim to that list or move the list into the task and delete the file.")
row("plat-legacy-removal-vocab","Retire the blank=/flush=/uninstall= shim and its doc trail","2026-09-03","queued","platform",
    refs="tasks/run-mode.yml · main.yml (import) · CLAUDE.md (3 legacy mentions) · tools/nos-stacks.sh",
    body="The shim's own header says delete after v0.10; v0.11-beta is tagged. One commit: drop the compat set_fact block + the blank,reset tags, the main.yml import stays for the real run-mode logic, and the three CLAUDE.md legacy mentions go in the same commit (the doc is the shim's exit condition — deleting one without the other reopens fee 07). nos-stacks.sh keeps refusing the tokens by name so an old script fails loud, not silent.")
row("cortex-s3-ann-experiment","S3: the organ's tuned ANN vs KEAP's default, over the agreed corpus","2026-09-03","next","cortex",
    refs="docs/archive/cortex-corpus-parallel.md §S2 exit + §4.3 · ~/.nos/cortex-corpus-diff.json · files/anatomy/cortex/server/cortex-ann.ts",
    body="S2 CLOSED 2026-09-03 (operator delegated the call, notification 382): 39 nights, 35 agree / 4 disagree, halted never — every disagreement was ingestion lag on a cadence (measured, incl. same-day self-heal on 09-03), none was content divergence. The nightly diff keeps running as a GUARD, not as evidence-gathering; adding S2 nights was the named furniture failure mode. "
         "S3 IS THE ASYMMETRY §4.3 protected all along: the organ ships tuned ANN defaults (float8 neighbors, maxNeighbors 20) over its OWN file, KEAP runs the plain default index — compare retrieval quality/latency over the agreed corpus. The caveat S3 inherits from the exit: corpora agree NIGHTLY, not intraday. Design the measurement before flipping any CORTEX_ANN_* env; the plan's §3 explains why a shared store would delete the experiment.")
row("keap-anchor-merge-never-heals","A wrong table anchor is immortal: syncCard merges, never replaces","2026-09-03","next","cortex",
    refs="~/keap/src/server/tables.ts:273 · state/keap-tables/ · tests/anatomy/test_table_anchors_resolve.py · inbox 362",
    body="MEASURED 2026-09-03: committed table defs carry the corrected anchors (nos.iiab.face, nos.host) and the live cards still lint broken (nos.face x3, nos.agents x2) AFTER a full converge, because syncCard unions prior+new anchors — deliberately, so a row-count bump cannot drop manually-added anchors — which makes a CORRECTION unexpressible: the stale anchor survives every re-POST forever. Third instance of one defect in one day: the roadmap seeder skips existing rows, provision-token only upserted (closed by --deactivate, ruling 4), and now anchors only accrete. "
         "FIX IS UPSTREAM (nos-keap, cross-repo-contracts doctrine applies): definition-sourced anchors need provenance so a def re-POST REPLACES def-sourced anchors and preserves manual ones — or the tables def-POST path gets an explicit replaceAnchors. Repo-side half already shipped: test_table_anchors_resolve.py refuses a COMMITTED anchor the lint knows is broken (live-conditional, retro-verified against nos.face). Until upstream lands, the five live broken-anchor findings stay HIGH in the lint and are librarian:judge-lint-queue food, not converge food.")
row("sec-origin-answers-anyone","The origin answers any source; scanners already hold the WAN IP","2026-09-03","next","platform",
    refs="infra-traefik-1 access log · roles/pazny.traefik/ · docs/security-baseline.md",
    body="MEASURED 2026-09-03, 24h of edge log: ~60 requests carry an IP-LITERAL Host (109.164.114.21) — mass-IPv4 scanners found the origin despite every DNS record being CF-proxied (checked: pazny.eu/auth/home/os all resolve to 188.114.9x). Probes include /cgi-bin/luci (OpenWrt admin RCE), /debug/pprof, /sbin/init payload paths, plus hostname noise (wp-login, .git/config, .aws/credentials, backup.sql) — 4669 no-router requests/24h, all dying in Traefik 404. SSH: zero remote logins in the entire wtmp (since May) and zero sshd contact in 24h — port 22 is NOT forwarded; router-admin probes land on Traefik, not the router UI. "
         "THE GAP: Traefik answers 80/443 from ANY source, so the CF proxy is anonymity, not a gate. THE TRAP that killed the obvious fix: Docker port-publish NATs every external source to 192.168.65.1 (measured in this log), so an ipAllowList of CF ranges CANNOT see real source IPs. The workable designs: (a) Cloudflare Authenticated Origin Pulls — CF presents a client cert, Traefik requires it on 443; NAT-immune; but required-mTLS breaks the smoke's loopback retry and any direct-LAN access to the public entrypoint, so it needs a carve-out or a second entrypoint; (b) proxy-protocol/real-IP plumbing through Docker, then ipAllowList; (c) router-side source filtering, which home routers rarely support. DECIDE (a) vs (b) before code; either way the operator router checklist stands independent: remote management OFF, UPnP OFF, forwards = exactly 80/443 to the Mac and nothing else.")
