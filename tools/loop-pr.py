...
def land(row: dict, *, base: str, gate_set: str, rejudge: bool,
    ...
    # read. That contract says such a proposal is never auto-accepted; until now
    # the column was stamped and read by nobody but a status reader, so a
    # gate-add that passed a judge set landed unattended — the gate-you-can-
    # satisfy-by-editing-the-gate class, through the front door. Held here,
    # before any judge is re-run or any branch is cut; an operator lands it by hand.
