...
- `n8n-pack.py` — the n8n pack contract in one tool: `lint` (CI gate §8), `encryption-key` (REM-202 adopt-or-mint), `sync` (post.yml harvester — upsert graphs keyed on meta.nos.id, inject nos-keap-rw), `watch` (Pulse exec reader: staleness vs cron, UNKNOWN when n8n is unreachable).
