...
#: STRUCTURAL ONLY — no status. status is table-owned and changes constantly; if
#: it lived here every seed run would rewrite the committed index and every pull
#: would conflict. The slug graph (slug/parent/track) is the stable part offline
#: gates actually need, and it changes only when a row is added or reparented.
_INDEX_FIELDS = ("slug", "parent", "track")
...
def write_index(rows: list[dict], repo_root: str) -> str:
    ...
    header = (
        "# GENERATED — the PUBLIC, content-free roadmap slug graph.\n"
        "# slug/parent/track ONLY; NO status/titles/bodies/refs (status is table-\n"
        "# owned and churny; the rest are the private ideas, in NOS_SEED_DIR).\n"
        "# Regenerated by tools/roadmap-seed.py from the private seed files; offline\n"
        "# gates read it to check a slug is real. Stable — changes only on add/reparent.\n"
    )
