...
def _task_prompt(weakness: dict, session_uuid: str) -> str:
    return (
        ...
        f"   WORKED EXAMPLE — the SHAPE to copy (a version_bump weakness): the "
        f"entire patch is ONE line in default.config.yml, e.g. "
        f"`traefik_version: \"3.1.2\"` -> `\"3.1.5\"`, and nothing else; the "
        f"proposal names that one file, that one-line diff, and the pin gate that "
        f"guards it. A proposal that edits many files, rewrites logic, or floats a "
        f"design is NOT bounded — shrink it to a single change like this, or "
        f"record (with evidence) that this weakness cannot be met in one bounded "
        f"patch. Small, concrete, one file: that is what clears the gate.\n"
