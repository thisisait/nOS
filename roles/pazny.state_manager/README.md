...
4. prints a compact summary and optionally pushes the state to Bone
...
| `nos_state_push_bone`          | `{{ install_bone | default(false) }}`                           | Best-effort POST to `/api/state`.                                    |
| `nos_state_bone_url`           | `http://localhost:{{ bone_port | default(8099) }}/api/state`    | Override if Bone runs elsewhere.                                     |
