...
Insert immediately **after** the `install_n8n:` line (IIAB section, ~line 131):
```yaml
install_nodered: false            # Node-RED - low-code flow automation      [requires: Docker]
```

## 2. `default.config.yml` — `authentik_oidc_apps` entry
Append into the `# -- Proxy-auth services (nginx forward auth, no native OIDC) --` block (after the WordPress proxy entry, before the helper vars):
```yaml
  - name: "Node-RED"
    slug: "nodered"
...
Add to `authentik_app_tiers:` (~line 1425, place alongside tier-2 entries — Node-RED function nodes grant shell-equivalent power, so manager tier):
```yaml
  nodered: 2
```
...
Insert into the `# IIAB roles` block (immediately after the `pazny.n8n render` line, ~line 70):
```yaml
