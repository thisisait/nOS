...
Ansible role for deploying **jsOS** — a self-hosted web desktop platform based on OS.js v3, running on Node.js via PM2. Part of [nOS](../../README.md).
...
4. Copies nOS server adapters (Bluesky auth, S3 VFS, per-user provisioning) from `files/jsos/`
...
- macOS with Homebrew
- Node.js via NVM, PM2 installed globally (handled by the main nOS playbook)
...
Secrets (`jsos_session_secret`, `jsos_db_password`) stay in the top-level `default.credentials.yml` so that `global_password_prefix` rotation propagates consistently across all nOS services.
