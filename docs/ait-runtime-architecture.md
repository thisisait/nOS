...
- **Operator-trigger UI** — POST `/api/v1/agents/<name>/sessions` with optional prompt + vault override. Runs the runner in a child process; UI polls `/api/v1/agent-sessions/<uuid>` for status.
