                       ...
                       └─ G-PROVISION-MIGRATED (migrations_authored.merged)
                          └─ --tags coexistence ─→ PROVISION
                                role=provisioned, active=0, source_migration_id set
                                  ├─ PROMOTE ─⇄─ role=primary  (atomic cutover, demotes prior)
...
6. **Toggle v17 primary — cutover RUNS the migration's data move.** Operator clicks **Toggle as primary** on v17 (typed `PRIMARY`). `actionTogglePrimary` → Bone `promote/v17` → `nos_coexistence.promote_track`: because v17's `source_migration_id` is set, it runs `nos_migrate action=apply migration_id=2026-06-15-postgresql-16-to-17` (the `pg_dumpall` dump from live v16 → restore into v17's empty cluster) and only after `verify` passes flips `active_track='v17'` (atomic state write + zero-downtime nginx/Traefik reload), sets v17 `role=primary active=1 read_only=0`, v16 `role=secondary read_only=1 ttl_until=+7d`. Emits **`coexistence_promote`** (+ the existing `coexistence_cutover`). `/upgrades` now shows postgresql **TWICE** (primary v17 / secondary v16) with the toggle reflecting the new primary; `upgrades_planned` flips to `applied`.

7. **Validate, then deactivate the secondary.** After confirming v17 serves traffic, click **Deactivate secondary** on v16 → `deactivate_track` stops the v16 container (data kept, override kept), `role=deactivated`. Emits **`coexistence_demote`**. (Rollback path stays open: re-promote v16 within the TTL.) Later, **Cleanup** reclaims v16 (`.backup-<ts>` rename) once the TTL elapses.
