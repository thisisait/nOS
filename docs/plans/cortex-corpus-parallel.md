...
| Backup: the store's "everything is derived" answer weakens | Still derivable — but derivation now depends on a **mounted removable volume**, not git. Re-read `nos-cortex-organ-design.md` §7 Q6 at S4, not here; S2 adds no row that a converge plus a mounted SSD cannot rebuild |
