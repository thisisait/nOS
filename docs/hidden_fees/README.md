...
| [43](43-healthy-inside-unreachable-outside.md) | Docker's host→container port forwarding fails partially; every service is healthy inside and unreachable from the host | ~40 min twice in one day, both spent proving the estate was fine; `nc`, `openssl` and the partial pattern each point somewhere else | **open** — a Docker restart cleared it once and not the second time; the smoke and hub gates still read `000` as an estate fault |
