...
**upgrade**, **coexistence** — flow between Ansible, Bone, Wing, and the host
...
| Suite | Recipe location | Engine task | Bone | Wing read | Wing write | State block | Events |
|---|---|---|---|---|---|---|---|
| **migrate** | `migrations/*.yml` | `tasks/pre-migrate.yml` | `/api/migrations/*` | `MigrationRepository` | `POST /api/v1/migrations/<id>/apply` → Bone | `migrations_applied[]` | `migration_start/_step_ok/_step_failed/_end` |
| **upgrade** | `upgrades/<service>.yml` | `tasks/upgrade-engine.yml` | `/api/upgrades/*` | `UpgradeRepository` | `POST /api/v1/upgrades/<svc>/<recipe>/apply` → Bone | `upgrades_applied[]` | `upgrade_start/_step_ok/_end` |
| **patch**   | `patches/PATCH-NNN.yml` | `tasks/apply-patches.yml` | `/api/patches/*` | `PatchRepository` | `POST /api/v1/patches/<id>/apply` → Bone | `patches_applied[]` | `patch_start/_step_ok/_step_failed/_end` |
| **coexist** | (stateful; no recipe files) | `tasks/coexistence-{provision,cutover,cleanup}.yml` | `/api/coexistence/*` | `CoexistenceRepository` | `POST /api/v1/coexistence/<svc>/{provision,cutover,cleanup}` → Bone | `coexistence{}` | `coexistence_provision/_cutover/_cleanup` |

All four suites follow the same contract on the wire: Bone is the only
component that ever invokes `ansible-playbook`. Wing never shells out;
it only reads from SQLite (fast) or proxies to Bone (slow-but-authoritative).
