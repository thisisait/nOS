...
| **A3b** | wing host-revert | **DEFERRED** to phase A3.5 — surfaced architectural gap: Traefik doesn't speak FastCGI, so Wing PHP-FPM on host (Homebrew) needs an HTTP front. Two paths: (a) keep wing-nginx as a thin sidecar container that bridges Traefik → host PHP-FPM, or (b) swap to FrankenPHP (single binary, native HTTP, no FastCGI). Operator decision pending. Until decided, Wing stays containerized — A2 already moved the source, that work isn't lost. | 1 d |
