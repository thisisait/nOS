...
>
> **Current gate:** A6.5 (Grafana thin-role pilot — doctrine proof) landed 2026-05-04. **Track Q is now unblocked.** A5 (contracts) landed 2026-05-04 — Bone/Wing OpenAPI + Wing DDL artifacts committed under `files/anatomy/skills/contracts/`, CI `contracts-drift` job catches API/schema regressions. Remaining PoC slices are A7 (gitleaks first-real plugin), A8 (conductor + agent runner), A9 (notifications), A10 (audit trail). Post-PoC expansion (additional plugins, additional agent profiles) remains incremental.
>
...
| **A5** | wing-exports | ✅ **2026-05-04**. Three scripts shipped: `files/anatomy/bone/bin/export-openapi.py` (FastAPI `app.openapi()` → YAML, 30 paths), `files/anatomy/wing/bin/export-openapi.php` (regex-parses RouterFactory + presenter docblocks → OpenAPI 3.1 YAML, 61 paths, 24 real summaries / 41 fallback), `files/anatomy/wing/bin/export-schema.php` (spawns `init-db.php` against a temp dir via `proc_open`, dumps `sqlite_master` → SQL DDL, 25 tables/1 view/32 indexes). Artifacts committed to `files/anatomy/skills/contracts/{bone.openapi.yml,wing.openapi.yml,wing.db-schema.sql}`. CI job `contracts-drift` regenerates all three and `diff -u`s vs committed (fails on drift). | 1 d |
