...
CONCEPTLESS = {
    ...
    "cnb-fx.currency": "ISO-4217 code — no currency concept exists — needs class.currency (same as invoice.currency)",
    "cnb-fx.amount": "quoted units per rate row (ČNB quotes JPY per 100) — no measure concept — needs measure.quantity",
    "cnb-fx.rate": "CZK per amount of currency — no money/measure concept — needs measure.amount (same as invoice.net_amount)",
    ...
    "invoice.rounding_amount": "PayableRoundingAmount — no money/measure concept — needs measure.amount",
    "invoice.document_kind": "invoice vs credit_note — class.kind is free; use it or class.document",
    "invoice.vat_regime": "standard vs reverse_charge — class.kind already used if document_kind takes it; needs class.vat_regime",
