...
def test_the_seed_key_order_is_the_dependency_order():
    ...
    # key order, so the SEED FILE is the order — no wrapper list to drift from
    # this gate. A different order 400s the converge (KEAP validates refTable
    # at create).
    assert list(_seed().keys()) == ORDER, (
        f"kolben-it.seed.yml key order {list(_seed().keys())} != the dependency "
    ...
    # repr and the include loop iterates CHARACTERS.
