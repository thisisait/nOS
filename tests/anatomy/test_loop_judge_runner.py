...
def test_a_completed_pytest_run_still_passes():
    ...
    verdict = J.run_gate_set(
        ...
        repo_root=REPO,
        spawn=_fake_spawn(
            **{"pytest": J.Completed(exit_code=0, stdout=f"{MEASURED_WORK['pytest-anatomy']} passed, 66 skipped in 351.00s\n")}
        ),
        probe=_always_true,
        sandbox_factory=lambda root: (root, "sha-fake", lambda: None),
    )
    assert verdict.result is J.Result.PASS, verdict.reason
    assert verdict.runs[0].work == MEASURED_WORK["pytest-anatomy"]
...
def test_pytest_never_runs_against_the_live_tree():
    ...
    def spy(argv, cwd, timeout_s):
        ...
        return J.Completed(exit_code=0, stdout=f"{MEASURED_WORK['pytest-anatomy']} passed in 351.00s\n")
...
def test_every_judge_in_a_set_observes_exactly_one_tree():
    ...
    def spy(argv, cwd, timeout_s):
        ...
        return {
            ...
            "pytest": J.Completed(exit_code=0, stdout=f"{MEASURED_WORK['pytest-anatomy']} passed in 351.00s\n"),
        }[_argv_key(argv)]
...
#: RE-DERIVED 2026-08-17 — FIRED again (3582 collected against the 3400 record),
#: grown by the apex font + web-root gates, the tool-result encoding gate and the
#: bound-loop gate. Fresh run: "3548 passed, 33 skipped in 258s". Third firing in
#: eleven days, every one by growth and none by a bad guess: the cadence this
#: ratchet replaces would have had to run three times in that window to match it.
...
#: to 4425 on 2026-08-30 when min_work went to 4370, and to 4496 on 2026-09-01
#: (min_work 4440)
