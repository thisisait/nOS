...
> is the bug. Numbered headings are the addresses; citations elsewhere keep `§N`.
> The verified Mermaid diagrams and the full narratives live in
> [`docs/loops-companion.md`](../../docs/loops-companion.md), under the same
> numbers — this file keeps the rules, the refusals and the missing-edge ranking.

## 0. Why this file exists

The first unattended night (2026-08-20→21) dropped an unjudged proposal between
`loop:propose` and `loop:drive`: nothing joined the step that makes a proposal
to the step that lands one. Closed `f3b34a19`. Full account: companion §0.

The general defect is why this document exists: **this estate has a gate for
every node and almost none for the edges.** Judges, ledger, weakness reader,
budget, driver, reviewer, cadence — each was built and pinned in isolation,
and each of those gates is genuinely good. No artifact stated the sequence, so
no gate could notice a missing step. This file states it; 7 ranks the edges
still missing; 8 says what an *edge gate* is and which to write first.
...
**Where they must not touch** (each refusal is enforced, not prose): SERE
**must not** edit its own oracle, engine, doctrine, secrets, edge surface
(`budget.py`, contract §5.2; gate `test_loop_budget_forbids_its_own_gates.py`).
It **must** never merge to `master` (`_refuse_master` in both driver and
reviewer), never push GitHub (`PROMOTE_ARGV = ["--apply"]`, never
`--push-github`), never converge, never run a removal, and never touch the
per-session AgentKit iteration loop (`agent_iterations` — a different loop,
contract §7 non-goal 1). The engine has no routable surface at all — loopback
Bone routes, no manifest entry (REM-144 doctrine).
...
landed-ness, and no step **may** write its own success (`tools/loop-status.py`
header; contract §3.5). Four credentialed identities traverse it — proposer,
engine judge-runner, driver/evaluator, reviewer — plus the operator; 4 maps
them. (State-machine diagram: companion §2.)
...
a desynced base (both directions), an unreachable forge ("not an empty one"),
and a branch tip it cannot prove it made (`_owns_remote_tip`, two proofs); the
reviewer touches only `fix/loop-*`, refuses on any NO, and *waits* on any
INDETERMINATE — an unanswered question is not a yes. A malformed patch is
`unusable`/`indeterminate`, never `fail`: a bad patch is not a bad idea
(`tools/loop-diff.py` moved the format burden off the model for exactly this).
...
The generalised rule (`loop-review.py` header): **whoever writes a change must
...
`/pulse` pause/unpause is an operator surface; a manifest **may** withdraw only
a pause whose reason is byte-identical to the one it declared (`72b909e3` —
gate `test_a_manifest_clears_only_its_own_pause.py`).

Operator acts the loop **must** never perform, enforced not promised: `forget`
...
accident: every reader **shall** exit 0 whatever it finds (a fact about the
estate must not be a build failure caused by nobody's commit), and every
unreadable source **must** be UNKNOWN, never green. `tools/nos-cc.sh` re-runs
these as panes — state, not scrollback. The verdict chain adds a ninth answer
no reader can fake: `nos-loop verdict --replay` re-runs the recorded argv
against the recorded tree and reproduces the recorded exit, work count and
stdout hash — a verdict that cannot be replayed is a claim.
