...
A deferral MUST never touch `status`. `tools/rem-status.py` filters strictly
on `status == "pending"` and `tools/discovery-scan.py` pattern-matches the
literal strings, so a new status value would delete deferred rows from the
one tool CLAUDE.md tells every operator and agent to consult first.

## 2. Why the third lane is the whole point

The roadmap asked to "act on CRITICAL/HIGH, batch the rest to a release
...
A release moves pins. It does nothing for a config change. Those 30 MUST NOT
be deferred to the next tag: that relabels actionable work as upstream-gated
and conserves no effort — while removing them from view for a quarter.

Two measurements that cut against the original framing: 40 of the 45 rows
...
## 3. What was refused, and why — this is the load-bearing part
...
nothing on day one and grows only as fast as probe work lands. The design MAY
return once probes exist; it MUST NOT ship as a floor that defers nothing.
...
prose. It SHALL stay a human call; what the estate MAY do is make the judgement
*legible* after the fact. Stop promising the automation.

## 4. A refusal added later, on the same evidence standard (2026-08-22)
