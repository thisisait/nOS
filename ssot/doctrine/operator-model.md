...
needs an `obs-` prefix and a slug-skip for that reason; the ideas table
already carries `shepherd` twice and `traccar` twice.
...
changes something — these SHALL be refused not because they are dangerous but
...
depends on. The operator MUST be asked, every time, and the asking SHALL
block.
...
none comes, the agent MAY act and MUST write down what was assumed.

**A flag, not a command.** When an agent needs the operator, the ask SHALL be a
...
decision costs someone's day, the ask MUST be a surface with a trail — and the
table in §2 is what that surface MUST implement.

## 4. What this does not license

- An agent MUST NOT widen its own decision rights by editing this file. The same
  rule as everywhere: a gate you can satisfy by editing the gate is not one.
- "Act on silence" for the roadmap SHALL NOT license acting on a **config**
