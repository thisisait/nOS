...
**The measurement that made this a re-labelling, not a refactor:** 33
DataTable DEFINITIONS live in nOS git (`state/keap-tables/*.table.yml`, gated
by `test_keap_table_concepts.py`); 24 of them are SYSTEM (estate tables nOS
...
1. **System vs user tables (definitions scope).** The definitions half covers
   only the code-declared SYSTEM tables (the 24). USER tables are KEAP-born (the
