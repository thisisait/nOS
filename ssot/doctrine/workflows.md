...
**`selection` — N alternatives, keep one — must not run** without a written
...
an exported name, a mount seam — it **shall** be sequential. Running a chain in
parallel parallelises the *guessing*, and reconciling costs more than the
wall-clock saved.

**Disjointness is a property of OUTPUT, not of input.** Different directories is
the right default for *construction*. A review panel whose lenses read the
...
once:** the brief **shall** be written in one place and passed *in the prompt*.
...
- **The judge is code; the proposer is a model; they never share an identity.**
  An agent that wrote code **must** not verify it, and a verifier **shall**
  receive the artefact, not the author's account of it.
...
clean trajectories and history outrank confidence. The irreversible **must**
not be automated.
...
— so they **shall** not share one gate.
...
2. **The ratchet is mandatory.** Every claimed improvement **shall** carry a
   gate shown to be RED before the fix. A system that **may** claim improvement
   unproven, will.
...
un-ignored). A loop **must** not propose a change to a file that is not in the
repository, and a judge **must** not be retro-red against a version never
committed.
